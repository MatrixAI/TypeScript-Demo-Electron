...
rec {
  ...
  node2nixDrv = env: runCommandNoCC "node2nix" {} ''
    mkdir $out
    cp ${./node2nix-generated-expressions}/${env}/* $out/
  '';
  # the shell attribute has the nodeDependencies, whereas the package does not
  node2nixProd = (
    (import (node2nixDrv "prod") { inherit pkgs nodejs; }).shell.override {
      src = src;
      dontNpmInstall = true;
    }
  ).nodeDependencies;
  node2nixDev = ((import (node2nixDrv "dev") { inherit pkgs nodejs; }).package.overrideAttrs (attrs: {
